  ...
  node(POD_LABEL) {
    stage('Build a Maven project') {
      ...
      container('maven') {
          ...
          sh "env;branch_prefix=\$(echo -n ${env.BRANCH_NAME} | sed -e 's|-.*||'); " +  'mvn -B sonar:sonar -Dsonar.login=4f607ae198fae2aa423bacd47959adeea6c36050 -Dsonar.projectKey=git_user_spy-$branch_prefix'
      }
    }
